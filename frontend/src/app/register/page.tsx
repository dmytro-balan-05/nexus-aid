                ...
                {step === 'register' ? (
                    <>
                        ...
                        <form onSubmit={handleRegister} className="flex flex-col gap-4">
                            <input className="border border-[var(--border)] bg-[var(--bg-primary)] text-[var(--text-primary)] p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-black text-sm"
                                   type="text" placeholder="Ваше імʼя" value={name} onChange={(e) => setName(e.target.value)} required />
                            <input className="border border-[var(--border)] bg-[var(--bg-primary)] text-[var(--text-primary)] p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-black text-sm"
                            ...
                            <button type="submit" disabled={isLoading}
                                    className="bg-black text-white p-2.5 rounded-xl font-bold hover:bg-gray-800 disabled:opacity-50 transition text-sm">
                    ...
                    <>
                        <p className="text-center text-sm text-[var(--text-secondary)] mb-2">Підтвердження email</p>
                        <p className="text-center text-xs text-[var(--text-secondary)] mb-6">
                            Код відправлено на <strong className="text-[var(--text-primary)]">{email}</strong>
                        </p>

                        <form onSubmit={handleVerify} className="flex flex-col gap-4">
                            <input
                                className="border border-[var(--border)] bg-[var(--bg-primary)] text-[var(--text-primary)] p-3 rounded-xl outline-none focus:ring-2 focus:ring-black text-center text-2xl font-bold tracking-[0.5em]"
                                type="text"
                                placeholder="000000"
                                maxLength={6}
                                value={code}
                        ...
                        <button onClick={handleResend}
                                className="w-full mt-3 text-sm text-center hover:opacity-70 transition"

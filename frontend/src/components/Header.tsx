        ...
        <header className="border-b border-gray-200 bg-white sticky top-0 z-50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-16 items-center">
                    ...
                    <div className="hidden md:flex items-center gap-3">
                        ...
                        {user ? (
                            <>
                                <Link href="/profile" className="flex items-center gap-3 hover:bg-gray-50 px-2 py-1 rounded transition cursor-pointer">
                            ...
                            <>
                                <Link href="/login" className="text-gray-900 font-medium hover:text-blue-600 px-3 py-2">
                                    Увійти
                                </Link>
                                <Link href="/register" className="bg-black text-white px-4 py-2 rounded-lg font-medium hover:bg-gray-800 transition">
                                    Реєстрація
                                </Link>
                            </>
                        )}
                    ...
                    <div className="md:hidden flex items-center">
                        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-gray-500 p-2">☰</button>
                    </div>
                </div>
            </div>
            ...
            {isMenuOpen && (
                <div className="md:hidden border-t border-gray-100 p-4 space-y-3 bg-white">
                    ...
                    {user?.role === 'admin' && (
                        <Link href="/admin/users" className="block font-medium">Адмін панель</Link>
                    )}
                    {!isHome && <QuickDonateButton compact />}
                    {user ? (
                        <>
